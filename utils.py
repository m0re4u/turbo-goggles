...
class Critic():
    ...
    def update_Qs(self, state, option, action, reward, done, terminations):
        ...
        self.Q_Omega_table[self.last_state,
                           self.last_option] += self.lr * tderror_Q_Omega

        tderror_Q_U = target - \
            self.Q_U(self.last_state, self.last_option, self.last_action)
        self.Q_U_table[self.last_state, self.last_option,
                       self.last_action] += self.lr * tderror_Q_U
